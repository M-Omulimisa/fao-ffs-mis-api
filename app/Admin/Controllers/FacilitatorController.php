...
class FacilitatorController extends AdminController
{
    ...
    protected function grid()
    {
        ...
        $grid->column('avatar', 'Photo')->image('', 48, 48);

        $grid->column('name', 'Facilitator')->display(function () {
            $name   = e($this->name ?: trim("{$this->first_name} {$this->last_name}"));
            $status = (string) $this->status;
            $badge  = $status === '1'
                ? '<span style="background:#d1fae5;color:#065f46;padding:2px 7px;border-radius:3px;font-size:11px;margin-left:6px">Active</span>'
                : '<span style="background:#fee2e2;color:#991b1b;padding:2px 7px;border-radius:3px;font-size:11px;margin-left:6px">Inactive</span>';
    ...
    protected function detail($id)
    {
        ...
        $show->divider();
        $show->field('district_id', 'District')->as(function ($id) {
            $loc = Location::find($id);
            return $loc ? $loc->name : '—';
        });
