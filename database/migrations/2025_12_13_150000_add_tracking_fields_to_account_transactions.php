...
class AddTrackingFieldsToAccountTransactions extends Migration
{
    ...
    public function up()
    {
        Schema::table('account_transactions', function (Blueprint $table) {
            ...
            $table->index('owner_type', 'idx_owner_type');
            $table->index('group_id', 'idx_group_id');
            $table->index('meeting_id', 'idx_meeting_id');
            $table->index('cycle_id', 'idx_cycle_id');
            $table->index('account_type', 'idx_account_type');
            $table->index('contra_entry_id', 'idx_contra_entry');
            
            // Composite index for common queries
            $table->index(['group_id', 'cycle_id'], 'idx_group_cycle');
            $table->index(['group_id', 'meeting_id'], 'idx_group_meeting');
            $table->index(['owner_type', 'group_id'], 'idx_owner_group');
            ...
            try {
                ...
                // Foreign keys might fail if tables don't exist or constraints already exist
                // Continue migration without foreign keys
            }
        });
    }
